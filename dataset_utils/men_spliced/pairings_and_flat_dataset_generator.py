...
def get_all_pairs_and_images(top_dir):
    clothing_cat_dirs = os.listdir(top_dir)
    clothing_cat_dirs = [(os.path.join(top_dir, dir), dir) for dir in clothing_cat_dirs]

    all_pairs = []
    all_individual_images = []
    all_image_file_names = []

    for cat_dir_path, cat_dir in clothing_cat_dirs:
        # print(f"Processing {cat_dir}")
        person_subfolders_list = os.listdir(cat_dir_path)
        for each_person_subfolder in person_subfolders_list:
            person_dir_path = os.path.join(cat_dir_path, each_person_subfolder)
            filename_dir = f"{cat_dir}_{each_person_subfolder}"
            ...
            all_pairs.extend(list(cart_prod))


    assert len(all_individual_images) == len(all_image_file_names), "Lists for paths to images and the corresponding list for the image file names should be equal"
...
if __name__ == "__main__":
    ...
    all_pairs, (all_images, all_image_filenames) = get_all_pairs_and_images(args.top_dir)
